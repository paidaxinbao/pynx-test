...
*/
__device__ void ObjectProbeZMultRAAR(const int i, complexf* obj, complexf* probe, complexf* psi, complexf* psiold,
                      float *dx, float *dy, signed char *sample_flag,
                      const int nb_proj, const int nb_z, const int nx, const int ny, const float beta)
{
  ...
  for(int iproj=0;iproj<nb_proj;iproj++)
  {
    ...
    for(int iz=0;iz<nb_z;iz++)
    {
      ...
      const complexf p = beta * probe[i + nx * ny * iz];
...
__device__ void ObjectProbeCohZMultRAAR(const int i, complexf* obj, complexf* probe, complexf* psi, complexf* psiold,
  ...
  for(int iproj=0;iproj<nb_proj;iproj++)
  {
    ...
    for(int iz=0;iz<nb_z;iz++)
    {
      ...
      for(int iprobe=0; iprobe<nb_probe; iprobe++)
        p += probe[i + nx * ny * (iprobe + nb_probe * iz)] * (beta * probe_coeffs[iprobe + nb_probe * (iz + nb_z * iproj)]);
