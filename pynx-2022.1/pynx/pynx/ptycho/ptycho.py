...
class PtychoData:
    ...
    def __init__(self, iobs=None, positions=None, detector_distance=None, mask=None,
        ...
        if iobs is not None:
            self.iobs = np.ascontiguousarray(fftshift(iobs, axes=(-2, -1)), dtype=np.float32)
            # This should not be necessary
            self.iobs[self.iobs < 0] = 0
            self.iobs_sum = self.iobs.sum()
            if mask is not None:
                self.mask = np.ascontiguousarray(fftshift(mask), dtype=np.int8)
...
class Ptycho:
    ...
    def __init__(self, probe=None, obj=None, background=None, data=None, nb_frame_total=None):
        ...
        if self._probe is not None:
            self._probe = np.ascontiguousarray(self._probe, dtype=np.complex64)
            if self._probe.ndim == 2:
                ny, nx = self._probe.shape
                self._probe = self._probe.reshape((1, ny, nx))

        if self._obj is not None:
            self._obj = np.ascontiguousarray(self._obj, dtype=np.complex64)
            if self._obj.ndim == 2:
        ...
        if self._background is not None:
            self._background = fftshift(np.ascontiguousarray(self._background, dtype=np.float32))
    ...
    def set_obj(self, obj):
        ...
        """
        self.from_pu()
        self._obj = np.ascontiguousarray(obj, dtype=np.complex64)
        if self._obj.ndim == 2:
            ny, nx = self._obj.shape
            self._obj = self._obj.reshape((1, ny, nx))
        self._timestamp_counter += 1
    ...
    def set_background(self, background):
        ...
        """
        self.from_pu()
        self._background = np.ascontiguousarray(fftshift(background), dtype=np.float32)
        self._timestamp_counter += 1
        self._cpu_timestamp_counter = self._timestamp_counter
    ...
    def set_probe(self, probe):
        ...
        """
        self.from_pu()
        self._probe = np.ascontiguousarray(probe, dtype=np.complex64)
        if self._probe.ndim == 2:
            ny, nx = self._probe.shape
            self._probe = self._probe.reshape((1, ny, nx))
        self._timestamp_counter += 1
    ...
    def get_psi(self, shift=False):
        """
        Get the current Psi array. Note that this is not not intended for an efficient
        data access, but only for occasional ones.

        :param shift: if True, the array will be shifted so that the array is centered
            on the frame center (and not the corner).
        :return: the raw psi numpy array for the currently processed stack of frames.
            Can also be None if no calculation took place.
        """
        if self._cpu_timestamp_counter < self._timestamp_counter:
            if self._timestamp_counter == self._cl_timestamp_counter:
                if has_attr_not_none(self, '_cl_psi'):
                    self._psi = self._cl_psi.get()

            if self._timestamp_counter == self._cu_timestamp_counter:
                if has_attr_not_none(self, '_cu_psi'):
                    self._psi = self._cu_psi.get()
        if isinstance(self._psi, np.ndarray) and shift:
            return fftshift(self._psi, axes=(-1, -2))
        return self._psi

    def get_iobs(self, i=None, shift=False):
        """
        Get the observed intensity array.

        :param i: the desired frame. if None, all data is returned.
        :param shift: if True, the array will be shifted so that the array is centered
            on the frame center (and not the corner).
        :return: a masked array of the observed intensity for frame i
        """
        iobs = self.data.iobs
        if i is not None:
            iobs = iobs[i]
        ...
        if shift:
            iobs = fftshift(iobs, axes=(-1, -2))
        return np.ma.masked_array(iobs, mask=iobs < 0)
