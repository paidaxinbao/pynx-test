...
params_generic = {'data': None, 'detector_distance': None, 'pixel_size_detector': None, 'wavelength': None,
                  ...
                  'support_autocorrelation_threshold': (0.09, 0.11), 'support_only_shrink': False, 'object': None,
                  ...
                  'flatfield': None, 'psf_filter': None, 'crop': None}
    ...
    def run_algorithm(self, algo_string, file_name=None, run_n=1):
        ...
        for algo in algo_split:
            ...
            #     # self.cdi = ScaleObj(method="F", verbose=True) * self.cdi
                ...
                if self.processing_unit.pu_language in ['cuda']:
        ...
        if k in ['live_plot', 'psf', 'save_plot']:
            ...
            if v is None:
                ...
                return False
                   ...
                   'iobs_saturation', 'support_fraction_min',
                   'support_fraction_max', 'support_threshold_auto_tune_factor',
                   'nb_run_keep_max_obj2_out']:
            self.params[k] = float(v)
            return True
        elif k in ['support_threshold', 'support_autocorrelation_threshold']:
            if ',' in v:
            ...
            return True
                   ...
                   'free_pixel_mask', 'support_formula', 'mpi', 'flatfield']:
            self.params[k] = v
            return True
        elif k in ['crop']:
            if len(v) > 1:
                # parameters per axis have been given, e.g. bin=122
                self.params['crop'] = [int(c) for c in v]
            else:
                self.params['crop'] = int(v)
        elif k in ['gpu']:
    ...
    def process_scans(self):
        ...
        for scan in vscan:
            ...
            try:
                ...
                elif default_processing_unit.pu_language == 'opencl':
                ...
                self.params['processing_language'] = default_processing_unit.pu_language
                self.params['processing_unit_name'] = gpuname
                s = ""
                if MPI is not None:
                    if self.mpi_size > 1:
                        s = "[MPI #%d/%d]" % (self.mpi_rank, self.mpi_size)
                print("Processing Unit init OK on %s [language=%s, GPU=%s] %s" %
                      (platform.node(), default_processing_unit.pu_language, gpuname, s))
                ...
                print('\n\n Caught exception for scan %s:\n   %s    \n'
                      'on %s [%s] %s' % (str(scan), str(ex), platform.node(), platform.platform(), s))
                sys.exit(1)
