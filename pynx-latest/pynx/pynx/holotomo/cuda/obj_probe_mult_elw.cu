...
__device__ void ObjectProbeZMult(const int i, complexf* obj, complexf* probe, complexf* psi,
      ...
      for(int iproj=0;iproj<nb_proj;iproj++)
      {
        ...
        for(int iz=0;iz<nb_z;iz++)
        {
          ...
          if(sample_flag[iproj]) o = bilinear(obj, dx[iz + iproj * nb_z]+ix, dy[iz + iproj * nb_z]+iy,
                                              iobj + nb_obj * iproj, nx, ny, false, false);
          // if(sample_flag[iproj]) o = obj[i + nx * ny * (iobj + nb_obj * iproj)];

          const complexf p = probe[i + nx * ny * (iprobe + nb_probe * iz)];
...
__device__ void ObjectProbeCohZMult(const int i, complexf* obj, complexf* probe, complexf* psi,
...
/** Copy the computed illumination in the Psi array for each projection.
* The 4-dimensional Psi stack is calculated, with dimensions:
*    projections, propagation distances, y, x
*
* This kernel function should be called for a 2D array, but will be applied to all z and projections.
...
__device__ void Probe2Psi(const int i, complexf* probe, complexf* psi, float *probe_coeffs,
                          const int nb_proj, const int nb_z, const int nb_probe,
                          const int nx, const int ny)
{
  // Coordinates in object array (origin at array center)
  const int ix = i % nx;
  const int iy = i / nx;
  // printf("Probe2Psi(%3d/%3d, %3d/%3d, %3d, %3d, %3d) \n", ix, nx, iy, ny,nb_proj, nb_z, nb_probe);
  ...
  for(int iproj=0;iproj<nb_proj;iproj++)
  {
    ...
    for(int iz=0;iz<nb_z;iz++)
    {
      ...
      psi[ipsi + nx * ny * (iz + nb_z * iproj)] = p;
    }
  }
}
