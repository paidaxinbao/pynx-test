...
* will be updated using (1+beta) * Psi - beta*Psi_old, and Psi will be replaced
* by beta * (Psi_old-Psi)
*
...
*/
__device__ float_%(NZN2)d Psi2ObjProbeDRAPRedN(const int i, complexf* obj, complexf* obj_old, complexf* probe,
                                               ...
                                               const float weight_empty, const float beta)
{
  ...
  { // Update object & probe
    ...
    for(int iz=0;iz<NZ;iz++)
    {
      ...
      const complexf ps = (1.0f + beta) * bilinear(psi, ix-dx[iz], iy-dy[iz], iz, nx, ny, false, true)
                                 - beta * bilinear(psiold, ix-dx[iz], iy-dy[iz], iz, nx, ny, false, true);
      complexf pr=0;
    ...
    for(int iz=0;iz<NZ;iz++)
    {
      ...
      const complexf ps = (1.0f + beta) * psi[ipsi1] - beta * psiold[ipsi1];
  ...
  { // sample_flag==0 => Update probe only
    ...
    for(int iz=0;iz<NZ;iz++)
    {
      ...
      const complexf rps = (1.0f + beta) * ps - beta * psold;
      ...
      // DRAP update of Psi
      psi[ipsi + nx * ny * iz] = beta * (psold - ps);
    }
  }
