...
"""Table of scattering factors from atoms and common ions, from the 1992 edition of the international tables for
Crystallography, Volume C, Mathematical, Physical and Chemical Tables, Edited by A.J.C. Wilson,
Kluwer Academic Publishers, Dordrecht/Boston/London, 1992.

The data was taken from the cctbx::eltbx::xray_scattering class
"""
...
def f_thomson(s, element):
    """
    Calculate the Thomson scattering factor for one atom or ion, for a given value of sin(theta)/lambda.
    
    :param s: :math:`sin(\\vartheta)/\\lambda`
    
    :param element: string of the element name ('H', 'Fe', 'Cu1+',...)
    
    :return: 
    """
    s2 = s ** 2
    a, b, c = d_fthomson_IT92[element]
    f = c
    for i in range(len(a)): f += a[i] * exp(-b[i] * s2)
    return f


def FThomson(s, element):
    warnings.warn("FThomson() is deprecated. Use f_thomson() instead.", DeprecationWarning)
    return f_thomson(s, element)