                                     ...
                                     const int nxy, const int nxystack, const int nbmode, const int npsi)
{
  ...
  for(int j=0; j<npsi; j++)
  {
    ...
    if(obs >=0)
    {
      ...
      {
        const complexf PO   = vPO  [ij + imode*nxystack];
        const complexf PdO  = vPdO [ij + imode*nxystack];
        const complexf dPO  = vdPO [ij + imode*nxystack];
        const complexf dPdO = vdPdO[ij + imode*nxystack];
             ...
             complexf *vdPO, complexf *vdPdO, const int nxy, const int nxystack, const int nbmode)
{
  ...
  for(int imode=0;imode<nbmode;imode++)
  {
    const complexf PO   = vPO  [i + imode*nxystack];
    const complexf PdO  = vPdO [i + imode*nxystack];
    const complexf dPO  = vdPO [i + imode*nxystack];
    const complexf dPdO = vdPdO[i + imode*nxystack];
    const complexf a = dPO + PdO;
