...
float2 CG_Poisson_Gamma(const int i, __global float *iobs, __global float2 *vPO, __global float2 *vPdO,
                        ...
                        const int nbmode, const int npsi)
{
  ...
  for(int j=0; j<npsi; j++)
  {
    ...
    if(obs >=0)
    {
      ...
      {
        const float2 PO   = vPO  [ij + imode*nxystack];
        const float2 PdO  = vPdO [ij + imode*nxystack];
        const float2 dPO  = vdPO [ij + imode*nxystack];
        const float2 dPdO = vdPdO[ij + imode*nxystack];
...
float4 CG_Poisson_Gamma4(const int i, __global float *iobs, __global float2 *vPO, __global float2 *vPdO,
             ...
             const int nxy, const int nxystack, const int nbmode)
{
  ...
  for(int imode=0;imode<nbmode;imode++)
  {
    const float2 PO   = vPO  [i + imode*nxystack];
    const float2 PdO  = vPdO [i + imode*nxystack];
    const float2 dPO  = vdPO [i + imode*nxystack];
    const float2 dPdO = vdPdO[i + imode*nxystack];
    const float2 a = dPO + PdO;
