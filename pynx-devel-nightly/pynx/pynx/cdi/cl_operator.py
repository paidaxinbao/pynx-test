...
class CLProcessingUnitCDI(CLProcessingUnit):
    ...
    def cl_init_kernels(self):
        ...
        self.cl_psf1 = CL_ElK(self.cl_ctx, name='cl_psf1',
                              operation="icalc[i] = iobs[i] >=0 ? iobs[i] / icalc[i] : scale",  # TODO: robustness ?
                              options=self.cl_options,
        ...
        self.cl_psf3 = CL_ElK(self.cl_ctx, name='cl_psf3',
                              operation="psf[i] *= fmax(d[i], 1e-6f)",  # TODO: robustness ?
                              options=self.cl_options,
...
class AutoCorrelationSupport(CLOperatorCDI):
    ...
    def op(self, cdi: CDI):
        ...
        pu.fft(tmp, tmp)
        thres = np.float32(pu.cl_max_red(tmp, wait_for=pu.ev).get() * self.threshold)
        pu.ev = []
    ...
    def op(self, cdi: CDI):
        ...
        if cdi._psf_f is None:
            ...
            if self.update_psf:
                ...
                s = pu.fft_scale(cdi._obj.shape)
                pu.cl_psf2(cl_icalc2_f, cl_icalc_f, s[0] * s[1], wait_for=pu.ev)
                ...
                if self.psf_filter is None:
                    pu.cl_psf3(cl_psf, cl_icalc)
                elif self.psf_filter.lower() == "tukey":
                    pu.cl_psf3_tukey(cl_psf, cl_icalc, np.float32(0.5), nx, ny, nz)
                else:
                    pu.cl_psf3_hann(cl_psf, cl_icalc, nx, ny, nz)
    ...
    def op(self, cdi: CDI):
        ...
        for i in range(self.nb_cycle):
            ...
            if self.filter is None:
                pu.ev = [pu.cl_psf3(cl_psf, cl_icalc, wait_for=pu.ev)]
            elif self.filter.lower() == "tukey":
                pu.ev = [pu.cl_psf3_tukey(cl_psf, cl_icalc, np.float32(0.5), nx, ny, nz, wait_for=pu.ev)]
            else:
                pu.ev = [pu.cl_psf3_hann(cl_psf, cl_icalc, nx, ny, nz, wait_for=pu.ev)]
