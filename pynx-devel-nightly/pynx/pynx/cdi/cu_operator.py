...
__all__ = ['default_processing_unit', 'AutoCorrelationSupport', 'FreePU', 'FT', 'IFT', 'FourierApplyAmplitude', 'ER',
           ...
           'InitPSF', 'PRTF', 'InitSupportShape', 'InitFreePixels', 'InitObjRandom', 'UpdatePSF']
...
class CUProcessingUnitCDI(CUProcessingUnit):
    ...
    def cu_init_kernels(self):
        ...
        # Using biweight does not help. Nor does using the free pixels trues intensity
        self.cu_psf1 = CU_ElK(name='cu_psf1',
                              # Use 'scale' if iobs==0, equivalent to iobs=icalc
                              operation="icalc[i] = iobs[i] >=0 ? iobs[i] / icalc[i] : scale",
        ...
        self.cu_psf3 = CU_ElK(name='cu_psf3',
                              operation="psf[i] *= fmaxf(d[i], 1e-6f)",  # TODO: robustness ?
                              options=self.cu_options,
                              arguments="float *psf, float *d")

        self.cu_psf3_hann = CU_ElK(name='cu_psf3_hann',
                                   operation="PSF3_Hann(i, psf, d, nx, ny, nz)",
                                   preamble=getks("cdi/cuda/psf_elw.cu"),
                                   options=self.cu_options,
                                   arguments="float* psf, float *d, const int nx, const int ny, const int nz")

        self.cu_psf3_tukey = CU_ElK(name='cu_psf3_tukey',
                                    operation="PSF3_Tukey(i, psf, d, alpha, nx, ny, nz)",
                                    preamble=getks("cdi/cuda/psf_elw.cu"),
                                    options=self.cu_options,
                                    arguments="float* psf, float *d, const float alpha,"
                                              "const int nx, const int ny, const int nz")
...
class AutoCorrelationSupport(CUOperatorCDI):
    ...
    def op(self, cdi: CDI):
        ...
        pu.fft(tmp, tmp)
        thres = np.float32(pu.cu_max_red(tmp).get() * self.threshold)
        cdi._cu_support = cua.zeros(cdi._cu_obj.shape, dtype=np.int8, allocator=pu.cu_mem_pool.allocate)
    ...
    def op(self, cdi: CDI):
        ...
        if cdi._psf_f is None:
            ...
            if self.update_psf:
                ...
                if not pu.use_vkfft:
                    # Need to recompute cu_icalc_f which may have be overwritten during the c2r transform,
                    # behaviour changed in cufft/cuda 11.1...
                    pu.fft(cu_icalc, cu_icalc_f)
                ...
                s = pu.fft_scale(cdi._obj.shape)
                pu.cu_psf2(cu_icalc2_f, cu_icalc_f, s[0] * s[1])
                ...
                if self.psf_filter is None:
                    pu.cu_psf3(cu_psf, cu_icalc)
                elif self.psf_filter.lower() == "tukey":
                    pu.cu_psf3_tukey(cu_psf, cu_icalc, np.float32(0.5), nx, ny, nz)
                else:
                    pu.cu_psf3_hann(cu_psf, cu_icalc, nx, ny, nz)
