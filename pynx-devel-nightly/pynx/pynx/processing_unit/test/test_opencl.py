...
class TestOpenCL(unittest.TestCase):
    ...
    @unittest.skipIf('cuda' in sys.argv or exclude_opencl, "OpenCL tests excluded")
    def test_gpyfft_import(self):
        msg = "Testing for gpyfft (needed for opencl FFT calculations)"
        try:
            import gpyfft
            import_ok = True
        except ImportError:
    ...
    @unittest.skipIf('cuda' in sys.argv or exclude_opencl, "OpenCL tests excluded")
    def test_gpyfft_fft_1D(self):
        msg = "Testing gpyfft fft execution (1D)"
        try:
            import numpy as np
            import pyopencl as cl
            import pyopencl.array as cla
            ...
            cl_ctx = CLProcessingUnit.get_context(opencl_gpu_device)
            cl_queue = cl.CommandQueue(cl_ctx)
            cl_psi = cla.empty(cl_queue, (128,), np.complex64)
    ...
    @unittest.skipIf('cuda' in sys.argv or exclude_opencl, "OpenCL tests excluded")
    def test_gpyfft_fft_2D(self):
        msg = "Testing gpyfft fft execution (2D)"
        try:
            import numpy as np
            import pyopencl as cl
            import pyopencl.array as cla
            ...
            cl_ctx = CLProcessingUnit.get_context(opencl_gpu_device)
            cl_queue = cl.CommandQueue(cl_ctx)
            cl_psi = cla.empty(cl_queue, (64, 64), np.complex64)
            gpyfft_plan = gpyfft.FFT(cl_ctx, cl_queue, cl_psi, None)
            cl_psi.fill(np.complex64(1))
            for ev in gpyfft_plan.enqueue(forward=True):
                ev.wait()
            for ev in gpyfft_plan.enqueue(forward=False):
                ev.wait()
    ...
    @unittest.skipIf('cuda' in sys.argv or exclude_opencl, "OpenCL tests excluded")
    def test_gpyfft_fft_2D_stacked(self):
        msg = "Testing gpyfft fft execution (stacked 2D)"
        fft_ok = False
        try:
            import numpy as np
            import pyopencl as cl
            ...
            cl_ctx = CLProcessingUnit.get_context(opencl_gpu_device)
            cl_queue = cl.CommandQueue(cl_ctx)
            cl_psi = cla.empty(cl_queue, (16, 64, 64), np.complex64)
            cl_psi.fill(np.complex64(1))
            gpyfft_plan = gpyfft.FFT(cl_ctx, cl_queue, cl_psi, axes=(-1, -2))
            for ev in gpyfft_plan.enqueue(forward=True):
                ev.wait()
            for ev in gpyfft_plan.enqueue(forward=False):
                ev.wait()
    ...
    @unittest.skipIf('cuda' in sys.argv or exclude_opencl, "OpenCL tests excluded")
    def test_gpyfft_fft_3D(self):
        msg = "Testing gpyfft fft execution (3D)"
        try:
            import numpy as np
            import pyopencl as cl
            import pyopencl.array as cla
            ...
            cl_ctx = CLProcessingUnit.get_context(opencl_gpu_device)
            cl_queue = cl.CommandQueue(cl_ctx)
            cl_psi = cla.empty(cl_queue, (64, 64, 64), np.complex64)
            cl_psi.fill(np.complex64(1))
            gpyfft_plan = gpyfft.FFT(cl_ctx, cl_queue, cl_psi, None)
            for ev in gpyfft_plan.enqueue(forward=True):
                ev.wait()
